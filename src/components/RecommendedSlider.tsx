	...
	const getTransitionStyles = (index: number): string => {
		...
		switch (windowSize) {
			...
			case "xLarge":
				...
				if (index === leftTertiaryBook || index === rightTertiaryBook)
					return "scale-50";
				if (index === leftSecondayBook || index === rightSecondaryBook)
					return "scale-75";
		...
		<div className="bg-red-50 pt-4 pb-8 box-border">
			<div className="flex flex-col text-center my-16">
				...
				<p className="w-2/3 my-6 md:my-10 m-auto text-slate-500">
					Unearth a world of books specifically curated to match your unique
					tastes and preferences! Dive into compelling narratives and discover
					hidden gems, meticulously selected to resonate with your literary
					appetite.
				</p>
			</div>
			<div className="min-w-full lg:w-11/12 flex relative md:m-auto md:py-4">
				<button
					...
					className="absolute z-10 left-2 md:left-4 lg:left-8 xl:left-12 bg-white shadow-md p-2 rounded m-2 top-1/3 transform -translate-y-1/2 hover:scale-110 active:scale-95 duration-300"
				>
					<svg
				...
				<div className="overflow-hidden">
					{" "}
					{/* Keep track of overflow! This took two days to debug! */}
					<div
						className="flex"
						style={{
							transform: `translateX(${counter * translationValue}%)`,
					...
					>
						{oldBooks.map((book, index) => (
							...
							>
								<img
									className={`m-auto w-1/2 md:m-0 md:w-full h-auto shadow-md rounded-md relative duration-500 ease-in-out ${transitionStyles[index]} `}
									src={book.image}
									alt=""
								/>
								<RecommendedBookDetails
									index={index}
									counter={counter}
									centerBookOffset={centerBookOffset}
									book={{
										title: book.title,
										price: book.price,
									}}
				...
				<button
					...
					className="absolute z-10 right-2 md:right-4 lg:right-8 xl:right-12 bg-white shadow-md p-2 rounded m-2 top-1/3 transform -translate-y-1/2 hover:scale-110 active:scale-95 duration-300"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 24 24"
