...
export default function RecomendedSlider(){
    ...
    return(
            <div className='bg-amber-50 pt-4 pb-8 box-border'>
                ...
                <div className='sm:min-w-full lg:w-11/12 flex relative m-auto py-4'>
                <button onClick={handleIncrement} className='absolute z-10 sm:left-2md:left-4 lg:left-8 xl:left-12 bg-white shadow-md p-2 rounded m-2 top-1/3 transform -translate-y-1/2 hover:scale-110 active:scale-95 duration-300'>
                 ...
                 <div className='overflow-hidden'> {/* Keep track of overflow! This took two days to debug! */}
                    <div className='flex transition-container ' style={{ transform: `translateX(${counter * translationValue}%)`, transition: 'transform 0.5s ease-in-out' }}>
                        {books.map((book, index) => (
                        <div key={index} className={`book-container sm:min-w-full md:min-w-1/3 lg:min-w-1/5`}>
                            <img className={`sm:m-auto sm:w-1/2 md:m-0 md:w-full h-auto shadow-md rounded-md relative duration-500 ease-in-out ${transitionStyles[index]} `} src={book.image} alt="" />
                ...
                <button onClick={handleDecrement} className='absolute z-10 sm:right-2 md:right-4 lg:right-8 xl:right-12 bg-white shadow-md p-2 rounded m-2 top-1/3 transform -translate-y-1/2 hover:scale-110 active:scale-95 duration-300'>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-amber-600 hover:text-amber-800 duration-200">
